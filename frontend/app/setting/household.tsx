...
const HouseholdManagementScreen = () => {
  ...
  // TESTING: Simple function to test with any token
  const testWithToken = async (token: string) => {
    try {
      await AsyncStorage.setItem("jwtToken", token);
      setJwtToken(token);
      Alert.alert("Token Set", "Testing with new token...");
      fetchFamilyMembers();
    } catch (error) {
      Alert.alert("Error", "Failed to set token");
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.card}>
        ...
        {/* TESTING: Button to quickly test with a token */}
        {__DEV__ && (
          <TouchableOpacity
